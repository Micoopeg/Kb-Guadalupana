    ...
    public partial class PendienteCertificacion : System.Web.UI.Page
    {
        ...
        public void llenargridviewcreditos()
        {
            using (MySqlConnection sqlCon = new MySqlConnection(conexiongeneral.cadenadeconexiongeneral()))
            {
                ...
                {
                    sqlCon.Open();
                    string query = "SELECT idpj_credito AS Credito, pj_nombrecliente AS Nombre, pj_status FROM pj_etapa_credito WHERE idpj_etapa = 1 AND pj_status IN ('Enviado','Reingreso') ";
