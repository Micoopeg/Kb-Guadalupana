    ...
    public partial class ProspectosFinalizados : System.Web.UI.Page
    {
        ...
        public void llenargridviewparacoordinadoragencia(string busqueda)
        {
            using (MySqlConnection sqlCon = new MySqlConnection(cn.cadenadeconexion()))
            {
                ...
                {
                    sqlCon.Open();
                    string QueryString = "select a.codcrminfoprospecto,b.crminfogeneral_prospectodpi,b.crminfogeneral_prospectonombrecompleto FROM crminfo_prospecto a INNER JOIN crminfogeneral_prospecto b INNER JOIN crmcontrol_prospecto_agente c INNER JOIN crmcontrol_ingreso d ON a.codcrminfogeneralprospecto=b.codcrminfogeneralprospecto AND c.codcrminfoprospecto=a.codcrminfoprospecto AND c.codcrmcontrolingreso=d.codcrmcontrolingreso WHERE a.codcrmsemaforoestado=1 AND d.crmcontrol_ingresosucursal!='telemercadeo' AND crminfogeneral_prospectonombrecompleto LIKE '%" + busqueda + "%';";
                    MySqlDataAdapter command = new MySqlDataAdapter(QueryString, sqlCon);
                    DataTable ds3 = new DataTable();
                    command.Fill(ds3);
                    gridviewprospectos.DataSource = ds3;
