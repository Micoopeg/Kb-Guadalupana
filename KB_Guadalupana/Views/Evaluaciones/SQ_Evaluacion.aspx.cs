        ...
        public void llenargridviewpersonas()
        {
            using (MySqlConnection sqlCon = new MySqlConnection(conexiongeneral.cadenadeconexiongeneral()))
            {
                try
                {
                    ...
                    string idpuesto = msq.obtenerpuesto(ususario);
                    sqlCon.Open();
                    string query = "SELECT B.sq_idcontrol AS Id, B.Nombre AS Nombre, C.puestodescrip AS Puesto, B.Autoevaluacion AS Autoevaluacion FROM sq_organigrama AS A INNER JOIN sq_controlevaluacion AS B ON B.Puesto = A.organisup INNER JOIN sq_puestoseval AS C ON C.codsqpuesto = B.Puesto WHERE A.codsqpuesto = '" + idpuesto + "'";
