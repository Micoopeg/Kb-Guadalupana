    ...
    public class ModeloEX
    {
        ...
        public DataTable llenarenviomesaarch()
        {
            ...
            using (MySqlConnection sqlCon = new MySqlConnection(conexiongeneral.cadenadeconexiongeneral()))
            {
                ...
                {
                    sqlCon.Open();
                    MySqlCommand command = new MySqlCommand("SELECT gcrd.gen_fechadesembolso,gaso.cifgeneral, gcrd.gen_numcredito, CONCAT_ws(' ',gaso.primer_nombre, gaso.segundo_nombre, gaso.primer_apellido, gaso.segundo_apellido) AS nombrecompleto , gcrd.gen_monto, extp.ex_nomtipo FROM gen_credito gcrd INNER JOIN gen_asociado gaso ON gaso.codgenasociado = gcrd.codgenasociado INNER JOIN ex_tipocredito extp ON extp.codextipocred = gaso.codgarantia INNER JOIN ex_temporal1 tmp ON tmp.Nocredito = gcrd.gen_numcredito INNER JOIN ex_envio exev  ON exev.Nocredito = tmp.Nocredito  WHERE  exev.estado = 8 AND exev.codexetapa = 5 ", sqlCon);
                    MySqlDataAdapter ds = new MySqlDataAdapter();
                    ds.SelectCommand = command;
                    ds.Fill(dt);
